...
try:
    ...
    # Setting init_default_scope=True (which is the default) ensures that
    # core mmdet3d modules like 'Det3DDataPreprocessor' are found.
...
class BEVFusionWrapper(nn.Module):
    ...
    def _init_bevfusion(self, config_path: str, checkpoint_path: Optional[str]) -> nn.Module:
        ...
        try:
            ...
            # By setting import_custom_modules=False, we prevent the config file from
            # re-importing 'bevfusion', which we have already handled at the top level.
            # This resolves the "already registered" error.
